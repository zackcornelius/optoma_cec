...
class pyCecClient:
  ...
  def SetConfiguration(self):
    self.cecconfig.strDeviceName   = "optomaUHD50"
  ...
  # initialise libCEC
  def InitLibCec(self, adapter = None):
  ...
  def ProcessCommandScan(self):
    ...
    for x in range(0, 15):
      if addresses.IsSet(x):
        ...
        strLog += "address:       " + str(".".join("%x" %physicalAddress)) + "\n"
...
def default_handler(cmd):
    print("%x > %x: %s - %s" % (cmd['src'], cmd['dst'], lib.lib.OpcodeToString(cmd['opcode']), cmd['args']))

def process_active_source(cmd):
    print("Active source command received, physical address: %s" % cmd['args'])
    proj.turn_on()

def process_image_view_on(cmd):
    print("Image view on received")
    proj.turn_on()

def process_standby(cmd):
    print("Requested to go to standby")
    proj.turn_off()
    # if cmd['dst'] != 0xf:
    lib.ProcessCommandTx("Of:36")


def process_give_physical_address(cmd):
    print("Requested to give physical address")
    lib.ProcessCommandTx("0f:84:00:00:00")

def process_give_device_power_status(cmd):
    print("Requested to give power status")
    proj_on = proj.power_status()
    lib.ProcessCommandTx("0%x:90:0%x" % (cmd['src'], 0 if proj_on else 1))

def process_give_osd_name(cmd):
    print("Requested to give OSD name")
    lib.ProcessCommandTx("0%x:47:6F:70:74:6F:6D:61:55:48:44:35:30" % cmd['src'])

def process_give_vendor_id(cmd):
    print("Requested to give vendor ID")
    lib.ProcessCommandTx("0%x:87:00:15:82" % cmd['src'])

opcode_handlers = {
    cec.CEC_OPCODE_ACTIVE_SOURCE: process_active_source,
    cec.CEC_OPCODE_IMAGE_VIEW_ON: process_image_view_on,
    cec.CEC_OPCODE_STANDBY: process_standby,
    cec.CEC_OPCODE_GIVE_PHYSICAL_ADDRESS: process_give_physical_address,
    cec.CEC_OPCODE_GIVE_DEVICE_POWER_STATUS: process_give_device_power_status,
    cec.CEC_OPCODE_GIVE_OSD_NAME: process_give_osd_name,
    cec.CEC_OPCODE_GIVE_DEVICE_VENDOR_ID: process_give_vendor_id,
}
...
def parse_command(cmd):
    ...
    pprint(cmd)
    command["src"] = int(cmd[0], 16)
    command['dst'] = int(cmd[1], 16)
    command['opcode'] = int(cmd[2:4], 16)
    command['args'] = cmd[4:]
    pprint(command)
    return command
...
def command_callback(cmd):
    ...
    handler = opcode_handlers.get(command['opcode'], default_handler)
    handler(command)
...
if __name__ == '__main__':
    ...
    proj = optoma.Optoma('/dev/ttyUSB0')

    # initialise libCEC and enter the main loop
    lib.InitLibCec(adapter = "/dev/serial/by-id/usb-Pulse-Eight_USB-CEC_Adapter_v7-if00")
